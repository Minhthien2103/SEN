...
class AudioToneAnalyzer:
    ...
    - 4 nhãn gốc: neu, hap, sad, ang (+ exc → hap)
    - Nếu confidence thấp → trả về "uncertain" thay vì đưa bừa label
    """
    CONFIDENCE_THRESHOLD = 0.40  # dưới ngưỡng này → uncertain

    # Map nhãn model → nhãn chuẩn
    LABEL_MAP = {
        "neu": "neutral",
        "hap": "happy",
        "exc": "happy",   # excited gộp vào happy
        "sad": "sad",
        "ang": "angry",
        "fea": "fearful",
    }
    ...
    # Giữ nguyên 8 nhãn chuẩn để tương thích với STT.py
    ...
    def analyze(self, audio: np.ndarray, sample_rate: int) -> dict:
        """
        Phân tích cảm xúc từ audio numpy array.

        Args:
            audio: numpy array âm thanh.
            sample_rate: sample rate của audio đầu vào.
        Returns:
            dict chứa xác suất các nhãn cảm xúc, dominant, human_readable, confidence.
        """
        ...
        try:
            ...
            if t.ndim > 1:
                t = torch.mean(t, dim=0)  # stereo → mono
            ...
            # 4. Map nhãn model → nhãn chuẩn, cộng dồn (exc + hap → happy)
            ...
            # Normalize lại (vì một số nhãn chuẩn không có trong model)
